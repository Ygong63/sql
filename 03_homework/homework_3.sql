...
SELECT customer_last_name, customer_first_name, SUM(p.quantity * p.cost_to_customer_per_qty) AS total_spent
FROM customer_purchases p
JOIN customer c ON c.customer_id = p.customer_id
GROUP BY p.customer_id, c.customer_last_name, c.customer_first_name
...
FROM
    customer_purchases
WHERE
    STRFTIME('%Y-%m', market_date) = '2019-04'
GROUP BY
    customer_id;
